...
const cacheResource = (url) => {
  fetch(url)
    ...
    .catch(error => {
      console.error(`Erreur lors de la mise en cache de la ressource ${url}:`, error);
    });
}
...
const detecteSoprod = async () => {
  ...
  const allTabs = await chrome.tabs.query({}).then((data) => data.filter(tab => !tab.url.includes("chrome")));
  console.log({allTabs})
  let isSoprodTab = {};
  ...
  allTabs.map(async (tab, i) => {
      ...
      console.log('tab length and i',allTabs.length - 1 ,i ,' userSoprod is undefined : ', userSoprod);
      const storageUser = await chrome.storage.sync.get('user');
      console.log('get user storage :; ',storageUser);
      console.log('is valide user soprod : ',storageUser.user, 'includes SO : ',!storageUser.user.includes('SO'));
      if (allTabs.length - 1 === i && userSoprod === undefined && !storageUser.user.includes('SO')) {
...
const checkDatas = () => {
  ...
  if(cmp === 2){
    ...
    if(global_data.dataChecker){
      ...
      console.log('________________+++++++++++++ user pour envoi vers indexDB : ',global_data.user);
