...
initDataChecker = () => {
  ...
  global_ratio_scores =
    size_scores.length > 0
      ? Number(
          (ratio_scores.reduce((a, b) => a + b) / ratio_scores.length).toFixed(
            2
          )
        )
      : 5;
  ...
  const global_alt_scores =
    alt_scores.length > 0
      ? Number((altScore.reduce((a, b) => a + b) / altScore.length).toFixed(2))
      : 5;
  ...
  const globalScoreWebTotal = Number(
    (globalScoreWeb.reduce((a, b) => a + b) / globalScoreWeb.length).toFixed(2)
  );
  console.log({ WebData }, { CDPData });
  const globalScoreCDPTotal = Number(
    (globalScoreCDP.reduce((a, b) => a + b) / globalScoreCDP.length).toFixed(2)
  );
