if (document.querySelector("#dm")) {
    ...
    let navInDecode = [];
    navInDecode.push(finalNavIn.map((node) =>{
      return {title:decodeURIComponent(escape(node.title)),alias:decodeURIComponent(escape(node.alias)),path:decodeURIComponent(escape(node.path)), inNavigation: node.inNavigation, subNab:node.subNab}
    }
    ...
    let navOutDecode = [];
    navOutDecode.push(finalNavOut.map((node) =>{
      return {title:decodeURIComponent(escape(node.title)),alias:decodeURIComponent(escape(node.alias)),path:decodeURIComponent(escape(node.path)), inNavigation: node.inNavigation, subNab:node.subNab}
    }
    ...
    const getAllPathValues = (obj) => {
      ...
      const traverse = (obj) => {
        for (var key in obj) {
          if (key === "path") {
            const link = new URL(window.location.origin + decodeURIComponent(escape(obj[key]))).href;
            links.push(link);
