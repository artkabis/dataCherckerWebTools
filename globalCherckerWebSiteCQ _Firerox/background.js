...
broswer.runtime.onInstalled.addListener(() => {
...
broswer.runtime.onMessage.addListener((message, sender, sendResponse) => {
...
broswer.runtime.onStartup.addListener(() => {
...
const toggle = (name, rule, value) => {
  ...
  broswer.declarativeNetRequest.updateEnabledRulesets(
    corsEnabled
      ? {
...
broswer.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
  // console.log("message service worker global :", request);
  // (request.action==='open_interface') && broswer.storage.sync.set({ dataChecker: request.data }, function(){
  ...
  //   });*/
  // });
  // const userRequest  = (request.user) && broswer.storage.sync.set({ user: request.user }, function(){
  ...
  if (request.corsEnabled !== undefined) {
    ...
    broswer.storage.sync.set({ user: request.user },()=>{
      userSoprod = request.user;
      console.log("user bg.js syncSet :", request.user);
      getUserMessage(userSoprod);
    });*/
    broswer.storage.sync.set({ corsEnabled: request.corsEnabled }, () => {
...
const once = () => {
  broswer.storage.sync.get("corsEnabled", (result) => {
    corsEnabled = result.corsEnabled;
    toggleCorsEnabled();
  });
};
// const getUserMessage=(user)=>{
//   broswer.storage.sync.get("user", (result) => {

//     console.log('function get user sync get : ',{user});
//   });
// }

broswer.runtime.onInstalled.addListener(once);
broswer.runtime.onStartup.addListener(once);
let user_soprod;
/****** check all tab */

const  detectOnotherInterface = async () => {
  const allTabs = await broswer.tabs.query({});
  console.log({allTabs});
  allTabs.forEach((tab, i) => {
    //const [tab] = await broswer.tabs.query({active: true, lastFocusedWindow: true});
  ...
  if (tab.url.includes("soprod")) {
    ...
    broswer.scripting.executeScript({
      ...
      function() {
        ...
        user_soprod = user;
        broswer.storage.sync.set({ user: user_soprod }, function () {
          console.log("---------------------storage sync user : ", { user });
          broswer.runtime.sendMessage({ user: user_soprod });
        });
      },
    });
  }
...
broswer.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  const checkDatas = () => {
    ...
    const interfacePopupUrl = broswer.runtime.getURL("interface.html");
          broswer.windows.create({
